...
from schemas.apartment_rent import (
    ApartmentRentCreate, ApartmentRentUpdate, ApartmentRentResponse, ApartmentRentWithParts
)
...
from crud import (
    get_apartments_sale, get_apartment_sale, create_apartment_sale, update_apartment_sale, delete_apartment_sale,
    get_apartments_rent, get_apartment_rent, create_apartment_rent, update_apartment_rent, delete_apartment_rent,
...
async def create_apartment_sale_endpoint(
    ...
    return create_apartment_sale(db=db, apartment=apartment)
...
async def update_apartment_sale(
    ...
    db_apartment = update_apartment_sale(db, apartment_id=apartment_id, apartment=apartment)
...
async def delete_apartment_sale(
    ...
    db_apartment = delete_apartment_sale(db, apartment_id=apartment_id)
    if db_apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    return db_apartment
...
async def create_apartment_rent_endpoint(
    ...
    return create_apartment_rent(db=db, apartment=apartment, listed_by_admin_id=current_admin.id)
...
async def update_apartment_rent_endpoint(
    ...
    db_apartment = update_apartment_rent(db, apartment_id=apartment_id, apartment=apartment)
    if db_apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    return db_apartment
...
async def delete_apartment_rent_endpoint(
    ...
    db_apartment = delete_apartment_rent(db, apartment_id=apartment_id)
    if db_apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    return db_apartment
...
async def create_apartment_part_for_apartment(
    ...
    """Create apartment part (admin only)."""
    apartment = get_apartment_rent(db, apartment_id=apartment_id)
    if apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    
    return create_apartment_part(db=db, part=part, admin_id=current_admin.id, apartment_id=apartment_id)
...
async def update_apartment_part_details(
    ...
    """Update apartment part (admin only)."""
    apartment = get_apartment_rent(db, apartment_id=apartment_id)
    if apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    
    db_part = update_apartment_part(db, part_id=part_id, part=part)
    if db_part is None:
...
async def delete_apartment_part_by_id(
    ...
    """Delete apartment part (super admin only)."""
    apartment = get_apartment_rent(db, apartment_id=apartment_id)
    if apartment is None:
        raise HTTPException(status_code=404, detail="Apartment not found")
    
    db_part = delete_apartment_part(db, part_id=part_id)
    if db_part is None:
        raise HTTPException(status_code=404, detail="Apartment part not found")
